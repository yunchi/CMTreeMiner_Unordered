						...
						const long & threshold)
{
	...
	******************************************************************/	
	...
	//for debug
	bool flag = false;
	if ( vCount == 7 && canonicalString.size() == 19 ) 
	{
		vector<short> temp(30);
		temp[0] = 7;
		temp[1] = 0;
		temp[2] = 20;
		temp[3] = 10;
	...
	******************************************************************/
	for ( long s = 1; s < occList.size(); s++ )
	{
		...
		if (flag)
		{
			...
			{
				cout << i+1 << ' ' << occList[s].nodeIndex[i] << endl; 
			}
			cout << endl;
			char c = getchar();
		}
				...
				while ( t != 0 )
				{
					if ( occList[s].nodeIndex[i-1] < t->v )
					{
						...
						if ( pos == neighbors.end() ) //if the vertex hasn't been used
						{
							...
							if(flag) cout << "i am in! added labels: " << legEVPair[0] << ' ' << legEVPair[1] <<endl;
						}
					}
					t = t->next;
				}
	...
	//for debug
	if ( commonLegs.size() != 0  && flag)
	{
		cout << "remaining commonLegs: " << endl;
		for ( short i = 0; i < commonLegs.size(); i++ )
			cout << commonLegs[i][0] << ' ' << commonLegs[i][1] << ' ' << commonLegs[i][2] << endl;
	}
