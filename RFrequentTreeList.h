...
public:
	...
	//this set of functions are defined for closed-pruning extension, DFCF
	void extensionExploreList4(const vector<ptrRFreeTree>& database, 
							  ostream& outFile,
							  vector<long> & frequency,
							  long & threshold,
							  vector<long> & checked, 
							  vector<long> & closed, 
							  vector<long> & maximal); //starting from list, find all frequent trees
